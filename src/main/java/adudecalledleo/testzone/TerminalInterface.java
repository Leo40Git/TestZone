...
public class TerminalInterface {
    ...
    private final BindingReader raw;
    private final LineReader reader;
    private final KeyMap<Integer> listMap;
    private final KeyMap<Integer> booleanMap;
    private final KeyMap<Integer> semVerMap;

    private final Prompter prompter;
    private Spinner spinner;

    public TerminalInterface(String appName) throws IOException {
        ...
        reader = LineReaderBuilder.builder().terminal(terminal).appName(appName).build();
        ...
        public String promptSemVer(String prompt) throws Exception {
            ...
            while (true) {
                ...
                if (input == 0 && currentSection > 0) { // Left arrow
                    ...
                    switch (currentSection) {
                        case 0: {
                            major = major > 0 ? major - 1 : 0;
                            break;
                        }
                        case 1: {
                            minor = minor > 0 ? minor - 1 : 0;
                            break;
                        }
                        case 2: {
                    ...
                    switch (currentSection) {
                        case 0: {
                            major = major * 10 + (input - 48);
                            break;
                        }
                        case 1: {
                            minor = minor * 10 + (input - 48);
                            break;
                        }
                        case 2: {
                            patch = patch * 10 + (input - 48);
                            break;
                        }
                        default: {
                            if (currentSection < buildThresh) {
